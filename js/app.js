...
function initOrganizeGame(){const game=document.querySelector('[data-organize-game]');if(!game)return;let selected=null,placed=0;const items=[...game.querySelectorAll('.sort-item')],msg=game.querySelector('[data-organize-message]');function reset(){selected=null;placed=0;items.forEach(i=>i.classList.remove('selected','placed'));game.querySelectorAll('.sort-zone').forEach(z=>z.classList.remove('correct','wrong'));msg.textContent='Escolha um objeto para começar.'}items.forEach(i=>i.addEventListener('click',()=>{items.forEach(x=>x.classList.remove('selected'));selected=i;i.classList.add('selected');msg.textContent='Agora toque no espaço correspondente.'}));game.querySelectorAll('.sort-zone').forEach(z=>z.addEventListener('click',()=>{if(!selected){msg.textContent='Primeiro escolha um objeto.';return}if(z.dataset.accept===selected.dataset.kind){selected.classList.remove('selected');selected.classList.add('placed');z.classList.add('correct');selected=null;placed++;msg.textContent=placed===items.length?'Tudo organizado. Respire e aproveite a sensação de ordem. 🌿':'Muito bem. Escolha o próximo objeto.'}else{z.classList.remove('wrong');void z.offsetWidth;z.classList.add('wrong');msg.textContent='Esse objeto combina melhor com outro espaço.'}}));game.querySelector('[data-reset-organize]').addEventListener('click',reset);reset()}
function initColorGame(){const game=document.querySelector('[data-color-game]');if(!game)return;const palette=game.querySelector('[data-color-palette]'),sequence=game.querySelector('[data-color-sequence]'),msg=game.querySelector('[data-color-message]');let shades=[],next=0;function reset(){shades=['#fff0b8','#f7d57a','#e8ad4d','#c87838','#8e472e','#542b27'];next=0;sequence.innerHTML='';palette.innerHTML='';[...shades].sort(()=>Math.random()-.5).forEach(c=>{const b=document.createElement('button');b.className='color-tile';b.style.background=c;b.dataset.index=shades.indexOf(c);b.setAttribute('aria-label','Peça de cor');b.addEventListener('click',()=>pick(b));palette.append(b)});msg.textContent='Comece pela peça mais clara.'}function pick(b){if(Number(b.dataset.index)!==next){msg.textContent='Observe com calma: existe uma tonalidade mais clara.';return}b.classList.add('used');const copy=b.cloneNode();copy.classList.remove('used');copy.disabled=true;sequence.append(copy);next++;msg.textContent=next===shades.length?'Degradê completo. Ficou harmonioso! 🎨':'Continue da mais clara para a mais escura.'}game.querySelector('[data-reset-colors]').addEventListener('click',reset);reset()}
...
document.addEventListener('DOMContentLoaded',()=>{initGuidedBreathing();initOrganizeGame();initColorGame();initBubbleGame();initHumor()});
